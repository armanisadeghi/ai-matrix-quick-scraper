...
class Scraper:
    ...
    async def clean_scrape(self):
        main_content = self.soup.find(attrs={'id': 'main-content'}) or self.soup
    ...
    async def get_tables(self):
        ...
        tables = self.soup.find_all('div', attrs={'role': 'table', 'class': 'dvmd_tm_table'})
    ...
    async def get_images_and_alt_text(self):
        ...
        for link in image_links:
            img_tags = self.soup.find_all('img', {'src': link}) or self.soup.find_all('img', {'data-src': link}) or self.soup.find_all('img', {'nitro-lazy-src': link})
            for img in img_tags:
    ...
    async def find_sitemap_link(self):
        ...
        if self.soup:
            ...
            meta_tags = self.soup.find_all('link', {'rel': 'alternate'}, href=True)
